...
/**
 * Convert mm to inches if dimension is > 400 (assumed mm) or explicitly marked mm
 */
const toInches = (val) => {
...
const resolveRowValues = (row = {}, index = 0) => {
    ...
    const particular = row.particularLabel ?? row.particular ?? row.windowType ?? 'Main Curtain';

    // Window Dimensions
    const o2oW = toInches(row.o2oWidth ?? row.outToOutWidth ?? row.o2o?.width);
    const o2oH = toInches(row.o2oHeight ?? row.outToOutHeight ?? row.o2o?.height);
    const f2fW = toInches(row.f2fWidth ?? row.frameToFrameWidth ?? row.f2f?.width ?? (!o2oW ? row.confirmedWidth ?? row.width : null));
    const f2fH = toInches(row.f2fHeight ?? row.frameToFrameHeight ?? row.f2f?.height ?? (!o2oH ? row.confirmedHeight ?? row.height : null));

    // Pelmet Dimensions
    const pO2oW = toInches(row.pelmetO2oWidth ?? row.pelmetOutOutWidth ?? row.pelmet?.o2oWidth ?? row.pelmet?.width);
    const pO2oDrop = toInches(row.pelmetO2oDrop ?? row.pelmetOutOutDrop ?? row.pelmet?.o2oDrop ?? row.pelmet?.drop);
    const pF2fW = toInches(row.pelmetF2fWidth ?? row.pelmetFrameFrameWidth ?? row.pelmet?.f2fWidth);
    const pF2fDrop = toInches(row.pelmetF2fDrop ?? row.pelmetFrameFrameDrop ?? row.pelmet?.f2fDrop);
    ...
    const rnft = Number(row.rnft ?? calc.rnft ?? 0);
    const romanSqft = Number(row.romanSqft ?? calc.romanSqft ?? 0);

    const totalParts = Number(row.totalParts ?? calc.totalParts ?? 0);
    ...
    const htPerPart = Number(row.heightPerPartM ?? calc.heightPerPartM ?? 0);
    const mtrsDrapes = Number(row.drapeMeters ?? calc.rawMetres ?? (roundedParts > 0 && htPerPart > 0 ? roundedParts * htPerPart : 0));
          ...
          <tbody>
            ${resolvedRows.map((r) => {
                ...
                if (r.floor && r.floor !== currentFloor) {
                    ...
                    floorBanner = `<tr class="floor-row"><td colspan="19">${escapeHtml(currentFloor)}</td></tr>`;
                }
 ...
 */
export const printConsumptionSheet = (data = {}, isBlank = false) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            console.error('printConsumptionSheet failed, falling back to window.print():', err);
            window.print();
            reject(err);
        }
    });
...
export default printConsumptionSheet;
