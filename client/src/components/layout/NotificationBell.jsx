  ...
  return (
    <div className="relative" ref={panel}>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        title="Notifications"
        className="relative p-2 text-slate-500 hover:text-slate-200 rounded-lg hover:bg-[ #836444] 800 transition"
      >
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-96 max-h-[28rem] overflow-y-auto panel shadow-2xl z-50">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 sticky top-0 bg-[ #836444] 900">
            <p className="text-xs font-semibold text-slate-200">
              Notifications {unread > 0 && <span className="text-slate-500">· {unread} unread</span>}
            </p>
            {unread > 0 && (
              <button
                type="button"
          ...
          {loading && <p className="px-4 py-6 text-xs text-slate-500 text-center">Loading…</p>}
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'w-full text-left flex gap-3 px-4 py-3 border-b border-slate-800/60 last:border-0 transition',
                  'hover:bg-[ #836444] 800/40',
                  !item.isRead && 'bg-blue-500/[0.04]'
                )}
              >
                <Icon className={cn('w-4 h-4 mt-0.5 shrink-0', tone)} />
                <div className="min-w-0 flex-1">
                  <p className={cn('text-xs truncate', item.isRead ? 'text-slate-400' : 'font-semibold text-slate-200')}>
                    {item.title}
                  </p>
                  {item.body && <p className="text-[11px] text-slate-500 mt-0.5 line-clamp-2">{item.body}</p>}
                  <p className="text-[10px] text-slate-600 mt-1">{relativeTime(item.createdAt)}</p>
                </div>
                {!item.isRead && <span className="w-1.5 h-1.5 rounded-full bg-blue-400 mt-1.5 shrink-0" />}
              </button>
            );
          })}
        </div>
