...
const ExecutionDrawingRequestPage = () => {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to load execution drawing request data');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <Panel className="p-12 text-center"><Loading text="Loading execution drawing request data..." /></Panel>
      ) : error ? (
        <ErrorState error={error} onRetry={handleLoad} />
      ) : stageItems.length === 0 ? (
        <Panel className="p-8 text-center">
          <EmptyState icon={FileText} title="No Drawing Requests Found" hint="Requests will appear here once they are raised." />
        </Panel>
      ) : (
        <Panel>
