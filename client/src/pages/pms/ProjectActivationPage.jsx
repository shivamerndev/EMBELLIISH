...
import { Briefcase, Eye, Pencil, Plus, AlertTriangle } from 'lucide-react';
...
const ProjectActivationEditModal = ({ item, onClose, onDone }) => {
  const [form, setForm] = useState({
    ...
    projectActivationDate: item?.projectActivationDate ? new Date(item.projectActivationDate).toISOString().slice(0, 10) : '',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatTile label="Completed" value={completedCount} sub="Activated projects" icon={Briefcase} tone="green" />
        <StatTile label="Client Approved" value={approvedCount} sub="Approved by client" icon={Briefcase} tone="amber" />
        <StatTile label="Pending KYC" value={stageItems.filter(i => i.kycBillingStatus === 'Pending').length} sub="Awaiting verification" icon={Briefcase} tone="orange" />
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-300">KYC Status</th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {stageItems.map((item, idx) => (
                  <tr key={item.id || item._id || idx} className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
                    <td className="px-4 py-3 text-slate-900 dark:text-slate-100 font-medium">{item.code || `Project ${idx + 1}`}</td>
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{item.projectActivationDate ? new Date(item.projectActivationDate).toLocaleDateString() : '—'}</td>
                    <td className="px-4 py-3"><Badge tone={item.clientApproval === 'Approved' ? 'green' : item.clientApproval === 'Rejected' ? 'rose' : 'slate'}>{item.clientApproval || 'Pending'}</Badge></td>
                    <td className="px-4 py-3"><Badge tone={item.kycBillingStatus === 'Verified' ? 'green' : 'amber'}>{item.kycBillingStatus || 'Pending'}</Badge></td>
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-300 text-xs">{item.assignedPcExecutionOwner || '—'}</td>
                    <td className="px-4 py-3"><Badge tone={item.status === 'Completed' ? 'green' : item.status === 'In Progress' ? 'blue' : 'slate'}>{item.status || 'Pending'}</Badge></td>
