...
const DesignFinalisationPage = () => {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to load design finalisation data');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        subtitle="Finalize design specifications, approve design variations, and confirm production-ready designs"
      />
      {loading ? (
        <Panel className="p-12 text-center"><Loading text="Loading design finalisation data..." /></Panel>
      ) : error ? (
        <ErrorState error={error} onRetry={handleLoad} />
      ) : stageItems.length === 0 ? (
        <Panel className="p-8 text-center">
          <EmptyState icon={Palette} title="No Design Records Found" hint="Design records will appear here once designs are finalized." />
        </Panel>
      ) : (
        <Panel>
